...
/*   Updated: 2025/07/17 17:18:21 by psmolin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_cmd *parse_or(t_token **current)
{
	...
	while (*current && (*current)->type == TOK_OR)
	{
		node = parse_add_node(*current, left, NULL);
		*current = (*current)->next;
		node->next_b = parse_and(current);
		left = node;
	}
	return (left);
}
...
static t_cmd *parse_and(t_token **current)
{
	...
	while (*current && (*current)->type == TOK_AND)
	{
		node = parse_add_node(*current, left, NULL);
		*current = (*current)->next;
		node->next_b = parse_pipe(current);
		left = node;
	}
	return (left);
}
...
static t_cmd *parse_pipe(t_token **current)
{
	...
	printf ("Parsing PIPE\n");
	printf ("Current token type: %d\n", (*current)->type);
	left = parse_word(current);
	while (*current && (*current)->type == TOK_PIPE)
	{
		node = parse_add_node(*current, left, NULL);
		*current = (*current)->next;
		node->next_b = parse_word(current);
		left = node;
	}
	return (left);
}
...
static t_cmd *parse_word(t_token **current)
{
	...
	{
		node = parse_add_node(*current, NULL, NULL);
		*current = (*current)->next;
		return (node);
	}
	return (NULL);
